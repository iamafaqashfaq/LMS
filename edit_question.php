                ...
                <div class="span9" id="content">
                     <div class="row-fluid">
					    <!-- breadcrumb -->	
									<ul class="breadcrumb">
										<?php
										$school_year_query = mysqli_query($con,"select * from school_year order by school_year DESC")or die(mysqli_connect_error());
                        ...
                        <div id="block_bg" class="block">
                            ...
                            <div class="block-content collapse in">
								...
								<?php
										...
										where quiz_id = '$get_id' and quiz_question_id = '$quiz_question_id'  order by date_added DESC ")or die(mysqli_connect_error());
										...
										<div class="control-group">
											...
											<div class="controls">			
												<select id="qtype" name="question_tpye" required>
													...
													<?php
													$query_question = mysqli_query($con,"select * from question_type")or die(mysqli_connect_error());
...
<div id="opt12">
	...
	<input name="correctt" <?php if($row['answer'] == 'False'){ echo 'checked'; }?> value="f" type="radio">False<br /><br />
		...
		if ($type  == '2'){
			...
			values('$get_id','$question',NOW(),'".$_POST['correctt']."','$type')")or die(mysqli_connect_error());
		}else{
	
		mysqli_query($con,"insert into quiz_question (quiz_id,question_text,date_added,answer,question_type_id) 
		values('$get_id','$question',NOW(),'$answer','$type')")or die(mysqli_connect_error());
		$query = mysqli_query($con,"select * from quiz_question order by quiz_question_id DESC LIMIT 1")or die(mysqli_connect_error());
		$row = mysqli_fetch_array($query);
		$quiz_question_id = $row['quiz_question_id'];
		
		mysqli_query($con,"insert into answer (quiz_question_id,answer_text,choices) values('$quiz_question_id','$ans1','A')")or die(mysqli_connect_error());
		mysqli_query($con,"insert into answer (quiz_question_id,answer_text,choices) values('$quiz_question_id','$ans2','B')")or die(mysqli_connect_error());
		mysqli_query($con,"insert into answer (quiz_question_id,answer_text,choices) values('$quiz_question_id','$ans3','C')")or die(mysqli_connect_error());
		mysqli_query($con,"insert into answer (quiz_question_id,answer_text,choices) values('$quiz_question_id','$ans4','D')")or die(mysqli_connect_error());
